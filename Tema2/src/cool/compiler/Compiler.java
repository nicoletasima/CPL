...
import cool.ast.*;
import cool.structures.ClassHierarchyPassVisitor;
...
//            });
        ...
        var astConstructionVisitor = new CoolParserBaseVisitor<ASTNode>() {
            ...
            public ASTNode visitProgram(CoolParser.ProgramContext ctx) {
                ...
                return new Program(visited, ctx.start, ctx);
            }
            ...
            public ASTNode visitClassDef(CoolParser.ClassDefContext ctx) {
                ...
                return new ClassDef(visited, ctx.type, ctx.parentType, ctx.start, ctx);
            }
            ...
            public Feature visitFuncDef(CoolParser.FuncDefContext ctx) {
                ...
                return new FuncDef(visited, instructions, ctx.type, ctx.name, ctx.start, ctx);
            ...
            public ASTNode visitVarDef(CoolParser.VarDefContext ctx) {
                if (ctx.init != null) {
                    return new VarDef((Expression) visit(ctx.init), ctx.name, ctx.type, ctx.start, ctx);
                }
                return new VarDef(ctx.name, ctx.type, ctx.start, ctx);
            }

            @Override
            public ASTNode visitMemberDef(CoolParser.MemberDefContext ctx) {
                if (ctx.init == null)
                    return new MemberDef(ctx.name, ctx.type, ctx.start, ctx);
                return new MemberDef((Expression) visit(ctx.init), ctx.name, ctx.type, ctx.start, ctx);

            }

            @Override
            public Expression visitInt(CoolParser.IntContext ctx) {
                return new Int(ctx.INTEGER().getSymbol(), ctx);
            }
            ...
            @Override
            public Expression visitId(CoolParser.IdContext ctx) {
                return new Id(ctx.ID().getSymbol(), ctx);
            }

            @Override
            public ASTNode visitMult(CoolParser.MultContext ctx) {
                return new Mult(ctx.MULT().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitDiv(CoolParser.DivContext ctx) {
                return new Div(ctx.DIV().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitPlus(CoolParser.PlusContext ctx) {
                return new Plus(ctx.PLUS().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitMinus(CoolParser.MinusContext ctx) {
                return new Minus(ctx.MINUS().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitLessOrEqual(CoolParser.LessOrEqualContext ctx) {
                return new LessOrEqual(ctx.LE().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitLess(CoolParser.LessContext ctx) {
                return new Less(ctx.LT().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitEqual(CoolParser.EqualContext ctx) {
                return new Equal(ctx.EQUAL().getSymbol(), (Expression) visit(ctx.left), (Expression) visit(ctx.right), ctx);
            }

            @Override
            public ASTNode visitNot(CoolParser.NotContext ctx) {
                return new Not(ctx.NOT().getSymbol(), (Expression) visit(ctx.op), ctx);
            }
            ...
            public ASTNode visitComplement(CoolParser.ComplementContext ctx) {
                return new Complement(ctx.COMPLEMENT().getSymbol(), (Expression) visit(ctx.op), ctx);
            }

            public ASTNode visitIsvoid(CoolParser.IsvoidContext ctx) {
                return new IsVoid(ctx.ISVOID().getSymbol(), (Expression) visit(ctx.op), ctx);
            }

            @Override
            public ASTNode visitNew(CoolParser.NewContext ctx) {
                return new New(ctx.NEW().getSymbol(), ctx.ID().getSymbol(), ctx);
            }

            @Override
            public ASTNode visitIf(CoolParser.IfContext ctx) {
                return new If(ctx.IF().getSymbol(), (Expression) visit(ctx.cond), (Expression) visit(ctx.thenBranch), (Expression) visit(ctx.elseBranch), ctx);
            }

            @Override
            public ASTNode visitWhile(CoolParser.WhileContext ctx) {
                return new While(ctx.WHILE().getSymbol(), (Expression) visit(ctx.cond), (Expression) visit(ctx.body), ctx);
            }
            ...
            public ASTNode visitBlock(CoolParser.BlockContext ctx) {
                ...
                return new Block(ctx.LBRACE().getSymbol(), visited, ctx);
            }

            public ASTNode visitCaseBranch(CoolParser.CaseBranchContext ctx) {
                return new CaseBranch((Expression) visit(ctx.body), ctx.name, ctx.type, ctx.CASE_BRANCH().getSymbol(), ctx);
            }

            public ASTNode visitCase(CoolParser.CaseContext ctx) {
                LinkedList<CaseBranch> visited = new LinkedList<>();

                for (var caseVar : ctx.cases) {
                    visited.add((CaseBranch) visit(caseVar));
                }
                return new Case(ctx.CASE().getSymbol(), (Expression) visit(ctx.cond), visited, ctx);
            }
            ...
            public ASTNode visitLet(CoolParser.LetContext ctx) {
                ...
                return new Let(ctx.LET().getSymbol(), visited, (Expression) visit(ctx.body), ctx);
            }
            ...
            public ASTNode visitImplicitDispatch(CoolParser.ImplicitDispatchContext ctx) {
                ...
                return new ImplicitDispatch(ctx.ID().getSymbol(), visited, ctx);
            }
            ...
            public ASTNode visitExplicitDispatch(CoolParser.ExplicitDispatchContext ctx) {
                ...
                return new ExplicitDispatch(ctx.DOT().getSymbol(), (Expression) visit(ctx.call), visited, ctx.name, ctx);
            }
        };
...
//
//                indent++;
//                printIndent(formal.getName().getToken().getText());
//                printIndent(formal.getType().getToken().getText());
//                indent--;
//                return null;
//            }
//
